...
/**
 * Expone el acceso (rol + pantallas) del usuario actual.
 *
 ...
 *
 * Valor inicial = mínimo privilegio (vendedor) mientras carga la cache.
 */
@HiltViewModel
class ScreenAccessViewModel @Inject constructor(
    ...
    val access: StateFlow<UserAccess> =
        observeScreenAccessUseCase().stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5_000L),
            initialValue = UserAccess.leastPrivilege(),
        )
}
